...
function setStyleProperties(placemarkNode, properties, styleByHash, styleIndex, styleMapIndex) {
    ...
    // var iconStyle = null
    let iconStyle = get1(placemarkNode, 'IconStyle');
    ...
    if (styleUrl) {
        ...
        if (style) {
            iconStyle = iconStyle||get1(style, 'IconStyle')
    ...
    let ovattr=get1(placemarkNode, 'OvAttr');
    if (ovattr){
...
function setOVAttrProperties(properties, style) {
    if (style){
        try {
            ...
            if (c){
                [...c].map(node=>{
                    properties[node.tagName]=nodeVal(node);
                });
            }
        }catch (e){
